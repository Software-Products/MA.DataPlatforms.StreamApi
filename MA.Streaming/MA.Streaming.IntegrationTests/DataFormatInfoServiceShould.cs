...
namespace MA.Streaming.IntegrationTests
{
    ...
    public class DataFormatInfoServiceShould : IClassFixture<KafkaTestsCleanUpFixture>
    {
        ...
        private readonly ThreadSafeInMemoryRepository<string, List<DataFormatRecord>> datasourceDataFormatsRepository;
        ...
        public DataFormatInfoServiceShould(ITestOutputHelper outputHelper, KafkaTestsCleanUpFixture _)
        {
            ...
            this.datasourceDataFormatsRepository = new ThreadSafeInMemoryRepository<string, List<DataFormatRecord>>();
            ...
            var typeNameProvider = new TypeNameProvider();
            this.dataFormatInfoService = new DataFormatInfoService(
                ...
                this.datasourceDataFormatsRepository,
        ...
        public void Add_Identifiers_To_Exist_DataFormat_Records_Identifier_Exist_Data_Format_Related_Packets_And_Update_The_Repository()
        {
            ...
            kafkaPublishHelper.PublishData(
                essentialTopic,
                GetPacketBytes(nameof(DataFormatDefinitionPacket), parameterDataFormatDefinitionPacket1.ToByteString()));
            kafkaPublishHelper.PublishData(
                essentialTopic,
                GetPacketBytes(nameof(DataFormatDefinitionPacket), eventDataFormatDefinitionPacket1.ToByteString()));
            kafkaPublishHelper.PublishData(
                essentialTopic,
                GetPacketBytes(nameof(DataFormatDefinitionPacket), parameterDataFormatDefinitionPacket2.ToByteString()));
            kafkaPublishHelper.PublishData(
                essentialTopic,
                GetPacketBytes(nameof(DataFormatDefinitionPacket), eventDataFormatDefinitionPacket2.ToByteString()));
            new AutoResetEvent(false).WaitOne(2000);
            //act
            this.dataFormatInfoService.Start();
            new AutoResetEvent(false).WaitOne(2000);
